defmodule BaguTest.AddNumbers do
  use Bagu.Tool,
...
defmodule BaguTest.MultiplyNumbers do
  use Bagu.Tool,
...
defmodule BaguTest.ToolAgent do
  use Bagu.Agent
...
defmodule BaguTest.MathPlugin do
  use Bagu.Plugin,
    description: "Provides math tools for Bagu agents.",
    tools: [BaguTest.MultiplyNumbers]
end

defmodule BaguTest.PluginAgent do
  use Bagu.Agent
...
defmodule BaguTest.ResearchSpecialist do
  ...
  def chat(_pid, message, opts \\ []) do
    ...
    depth = Map.get(context, Bagu.Subagent.depth_key(), 0)

    {:ok, "research:#{message}:tenant=#{tenant}:depth=#{depth}"}
  end
end

defmodule BaguTest.ReviewSpecialist do
  defmodule Runtime do
    use Jido.Agent,
...
defmodule BaguTest.OrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist

    subagent BaguTest.ReviewSpecialist,
      as: "review_specialist",
      description: "Ask the review specialist"
  end
end

defmodule BaguTest.PeerOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, target: {:peer, "research-peer-test"}
  end
end

defmodule BaguTest.ContextPeerOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, target: {:peer, {:context, :research_peer_id}}
  end
end

defmodule BaguTest.ContextPeerNoForwardOrchestratorAgent do
  use Bagu.Agent
...
defmodule BaguTest.WrongPeerOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, target: {:peer, "wrong-peer-test"}
  end
end

defmodule BaguTest.ForwardNoneOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, forward_context: :none
  end
end

defmodule BaguTest.ForwardOnlyOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, forward_context: {:only, [:tenant, "notify_pid"]}
  end
end

defmodule BaguTest.ForwardExceptOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, forward_context: {:except, ["secret"]}
  end
end

defmodule BaguTest.StructuredOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, result: :structured
  end
end

defmodule BaguTest.MissingPeerOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.ResearchSpecialist, target: {:peer, "missing-peer-test"}
  end
end

defmodule BaguTest.SlowSpecialist do
  defmodule Runtime do
    use Jido.Agent,
...
defmodule BaguTest.TimeoutOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.SlowSpecialist, timeout: 20
  end
end

defmodule BaguTest.InvalidResultSpecialist do
  defmodule Runtime do
    use Jido.Agent,
...
defmodule BaguTest.InvalidResultOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.InvalidResultSpecialist
  end
end

defmodule BaguTest.InterruptSpecialist do
  defmodule Runtime do
    use Jido.Agent,
...
defmodule BaguTest.InterruptOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.InterruptSpecialist
  end
end

defmodule BaguTest.InvalidInterruptSpecialist do
  defmodule Runtime do
    use Jido.Agent,
...
defmodule BaguTest.InvalidInterruptOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.InvalidInterruptSpecialist
  end
end

defmodule BaguTest.StartFailureSpecialist do
  defmodule Runtime do
    use Jido.Agent,
...
defmodule BaguTest.StartFailureOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.StartFailureSpecialist
  end
end

defmodule BaguTest.StartIgnoreSpecialist do
  defmodule Runtime do
    use Jido.Agent,
...
defmodule BaguTest.StartIgnoreOrchestratorAgent do
  use Bagu.Agent
  ...
  capabilities do
    subagent BaguTest.StartIgnoreSpecialist
  end
end

defmodule BaguTest.StartTripleSpecialist do
  defmodule Runtime do
    use Jido.Agent,
  ...
  def start_link(opts \\ []) do
    with {:ok, pid} <- Bagu.start_agent(Runtime, opts) do
      {:ok, pid, %{mode: :triple}}
    end
  end
...
defmodule BaguTest.StartTripleOrchestratorAgent do
  use Bagu.Agent
