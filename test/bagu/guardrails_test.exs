defmodule BaguTest.GuardrailsTest do
  ...
  test "wraps Bagu.Guardrail with published names" do
    assert Bagu.Guardrail.validate_guardrail_module(SafePromptGuardrail) == :ok
    assert {:ok, "safe_prompt"} = Bagu.Guardrail.guardrail_name(SafePromptGuardrail)

    assert {:ok, ["safe_prompt", "safe_reply"]} =
             Bagu.Guardrail.guardrail_names([SafePromptGuardrail, SafeReplyGuardrail])
  end
  ...
  test "accepts request-scoped module, MFA, and function guardrails" do
    runtime_fun = fn %Bagu.Guardrails.Input{} = input ->
    ...
    assert %{
           ...
           } = tool_context[:__bagu_guardrails__]
  end
  ...
  test "rejects malformed request-scoped guardrail specs with a validation error" do
    assert {:error, %Bagu.Error.ValidationError{} = error} =
             Bagu.Agent.prepare_chat_opts([guardrails: [1, 2]], nil)
  ...
  test "tool guardrails attach a runtime callback that interrupts before tool execution" do
    ...
    assert {:interrupt, %Bagu.Interrupt{kind: :approval, message: "Large calculations require approval"}} =
             callback.(%{
  ...
  test "translates input guardrail interrupts from Bagu.chat and runs interrupt hooks" do
    assert {:ok, pid} = GuardrailedAgent.start_link(id: "guardrailed-agent-test")
    test_pid = self()

    try do
      assert {:interrupt, %Bagu.Interrupt{kind: :approval}} =
               Bagu.chat(pid, "hello",
