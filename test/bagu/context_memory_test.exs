defmodule BaguTest.ContextMemoryTest do
  ...
  test "accepts context keyword lists and normalizes them to internal tool_context" do
    assert {:ok, opts} =
             Bagu.Agent.prepare_chat_opts([context: [tenant: "acme", locale: "en-US"]], nil)
  ...
  test "rejects malformed context lists with a structured validation error instead of raising" do
    assert {:error, %Bagu.Error.ValidationError{} = error} =
             Bagu.Agent.prepare_chat_opts([context: [1, 2]], nil)

    assert error.field == :context
    assert error.details.reason == :expected_map
    assert Bagu.format_error(error) == "Invalid context: pass `context:` as a map or keyword list."
  end
  ...
  test "validates required runtime context while applying schema defaults" do
    ...
    assert {:ok, opts} =
             Bagu.Agent.prepare_chat_opts([context: %{account_id: "acct_123"}], config)

    assert Keyword.get(opts, :tool_context) == %{account_id: "acct_123", tenant: "demo"}

    assert {:error, %Bagu.Error.ValidationError{} = error} =
             Bagu.Agent.prepare_chat_opts([context: %{}], config)

    assert error.details.errors == %{account_id: ["is required"]}
  end

  test "Bagu.chat validates context through the running agent schema" do
    assert {:ok, pid} = ContextAgent.start_link(id: "context-schema-chat")

    assert {:error, %Bagu.Error.ValidationError{} = error} =
             Bagu.chat(pid, "hello", context: %{tenant: 123})
  ...
  test "merges default agent context into runtime requests" do
    ...
    assert Bagu.Context.strip_internal(params.tool_context) == %{tenant: "demo", channel: "test"}
  ...
  test "retrieves and captures conversation memory across turns" do
    ...
    memory = params.tool_context[Bagu.Memory.context_key()]
    ...
    assert params.runtime_context[Bagu.Memory.context_key()].namespace ==
    ...
    assert get_in(agent.state, [:requests, "req-memory-2", :meta, :bagu_memory, :namespace]) ==
             "agent:memory_agent:context:session:#{session}"
  end
  ...
  test "rejects public tool_context in favor of context" do
    assert {:error, %Bagu.Error.ValidationError{} = error} =
             Bagu.Agent.prepare_chat_opts([tool_context: %{actor: %{id: "user-1"}}], nil)
  ...
  test "rejects public tool_context in chat helpers" do
    ...
    try do
      assert {:error, %Bagu.Error.ValidationError{} = chat_error} =
               ChatAgent.chat(pid, "Hello", tool_context: %{tenant: "acme"})

      assert {:error, %Bagu.Error.ValidationError{} = bagu_error} =
               Bagu.chat(pid, "Hello", tool_context: %{tenant: "acme"})

      assert chat_error.details.reason == :use_context
      assert bagu_error.details.reason == :use_context
    after
