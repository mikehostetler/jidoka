defmodule BaguTest.SubagentsTest do
  ...
  test "runs ephemeral subagents through generated tool modules and forwards public context only" do
    ...
    context = %{
      ...
      :__bagu_hooks__ => %{before_turn: [:demo]},
      Bagu.Subagent.depth_key() => 0
    }
    ...
    assert forwarded_context[Bagu.Subagent.depth_key()] == 1
    refute Map.has_key?(forwarded_context, :memory)
    refute Map.has_key?(forwarded_context, :__bagu_hooks__)
  end
  ...
  test "supports persistent peer subagents with static ids" do
    ...
    try do
      ...
      assert Bagu.whereis("research-peer-test") == pid
    after
  ...
  test "rejects persistent peers whose runtime module does not match the configured subagent" do
    ...
    try do
      ...
      assert error.details.cause ==
               {:peer_mismatch, BaguTest.ResearchSpecialist.Runtime, BaguTest.ReviewSpecialist.Runtime}
    after
      :ok = Bagu.stop_agent(pid)
    end
  end
  ...
  test "normalizes timeout failures and stops ephemeral children" do
    ...
    assert [%{child_id: child_id, outcome: {:error, {:timeout, 20}}}] =
             Bagu.Subagent.request_calls(self(), request_id)
  ...
  test "normalizes child interrupts" do
    ...
    assert %Bagu.Interrupt{kind: :approval, message: "Need approval"} = error.details.cause
  end
  ...
  test "accepts supervisor-style triple start returns" do
    ...
    assert [%{child_id: child_id, outcome: :ok}] = Bagu.Subagent.request_calls(self(), request_id)
    refute_agent_running(child_id)
  end
  ...
  test "falls back to live subagent metadata when request state has not been updated yet" do
    ...
    assert [
             ...
             Bagu.Subagent.request_calls(self(), "req-subagent-meta-live-1")
  end
      ...
      spawn(fn ->
        assert {:ok, %{result: "research:Collect from exiting process:tenant=exited:depth=1"}} =
                 research_tool.run(%{task: "Collect from exiting process"}, %{
                   Bagu.Subagent.server_key() => parent,
                   Bagu.Subagent.request_id_key() => request_id,
                   tenant: "exited"
                 })
  ...
  test "returns recorded subagent calls in invocation order" do
    context = %{
      Bagu.Subagent.server_key() => self(),
      Bagu.Subagent.request_id_key() => "req-subagent-order-1",
      tenant: "ordered"
    }
    ...
    assert [
             ...
             %{name: "research_agent"}
           ] = Bagu.Subagent.request_calls(self(), "req-subagent-order-1")
  end

  defp refute_agent_running(id, attempts \\ 10)

  defp refute_agent_running(id, 0), do: refute(Bagu.whereis(id))

  defp refute_agent_running(id, attempts) do
    case Bagu.whereis(id) do
      nil ->
        assert true
