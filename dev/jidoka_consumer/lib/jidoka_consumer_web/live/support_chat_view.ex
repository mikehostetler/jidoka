defmodule JidokaConsumerWeb.SupportChatView do
  @moduledoc """
  Phoenix-facing view adapter for the support note agent.

  This is the spike boundary: the Jidoka agent owns execution and `Jido.Thread`
  ...
  @agent JidokaConsumer.SupportNoteAgent
  ...
  def snapshot(pid, session, opts \\ []) when is_pid(pid) and is_map(session) do
    with {:ok, projection} <- Jidoka.Agent.View.snapshot(pid, opts) do
      {:ok,
       %{
         agent_id: agent_id(session),
  ...
  @doc "Runs the Jidoka chat turn for a LiveView submit event."
  ...
  @doc "UI hook executed after a Jidoka chat turn returns."
  ...
  defp apply_result(view, {:error, reason}) do
    %{view | status: :error, error: Jidoka.format_error(reason)}
  end
