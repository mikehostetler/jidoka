# Jidoka Project Review

Date: 2026-04-21

Baseline commit: `9b9db9a test: add live filesystem mcp coverage`

Dependency check: Jidoka now compiles and passes tests against Hex `jido 2.2.0`
and Hex `jido_ai 2.1.0`. `ash_jido` is sourced from GitHub rather than the
local reference checkout.

## Executive Summary

Jidoka has crossed from concept spike into a coherent first-pass package. The
...
The most important conclusion is that Jidoka should not broaden again yet. The
...
| Live MCP coverage | filesystem MCP server syncs tools into a running Jidoka agent |

## TODO Alignment

The high-level checklist in `TODO.md` is accurate. Jidoka has a real first-pass
...
| `tools` | Implemented | `Jidoka.Tool` provides a narrow wrapper over `Jido.Action`. |
...
Jidoka now has a clear public mental model. The package feels like an agent
...
- The debug story is now Jidoka-shaped instead of raw Jido log spam.
...
Jidoka now depends on Hex releases for `jido` and `jido_ai`. This is healthier
...
`jido_ai == 2.0.0-rc.0`), so Jidoka still needs direct `jido` and `jido_ai`
overrides to keep the graph on current Hex Jido releases.

### MCP Sync Boundary

Jidoka owns `Jidoka.MCP.Sync` as the default sync adapter and exposes `Jidoka.MCP` as
...
Jidoka now has many first-pass concepts. That is acceptable for a spike, but entry
...
`jido_memory` compatibility fixes upstream, then reduce Jidoka-owned shims where
possible.

2. Harden MCP as a client/tool source.

Keep the scope narrow: configured MCP endpoints sync tools into Jidoka agents.
...
Keep improving `Jidoka.inspect_agent/1`, `Jidoka.inspect_request/1`, and `mix jidoka
...
Jidoka is ready for a serious baseline review. It is not ready to be treated as a
