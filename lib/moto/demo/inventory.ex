defmodule Moto.Demo.Inventory do
  @moduledoc false

  @detail_levels [:debug, :trace]
  @row_width 13

  @spec print_compiled(String.t(), module(), Moto.Demo.Debug.log_level(), keyword()) :: :ok
  def print_compiled(title, agent_module, level, opts \\ []) when is_atom(agent_module) do
    print_definition(title, agent_module.__moto__(), level, opts)
  end

  @spec print_imported(String.t(), Moto.ImportedAgent.t(), Moto.Demo.Debug.log_level(), keyword()) ::
          :ok
  def print_imported(title, %Moto.ImportedAgent{} = agent, level, opts \\ []) do
    print_definition(title, Moto.ImportedAgent.definition(agent), level, opts)
  end

  @spec print_definition(String.t(), map(), Moto.Demo.Debug.log_level(), keyword()) :: :ok
  ...
  defp format_ash(%{ash_resources: resources}) when is_list(resources) do
    case Moto.Agent.AshResources.expand(resources) do
