defmodule Moto.Agent.Verifiers.VerifyHooks do
    ...
    |> Enum.reduce_while({:ok, default_seen()}, fn
      hook_ref, {:ok, seen} ->
        ...
        cond do
          ...
          true ->
            case Moto.Hooks.validate_dsl_hook_ref(stage, hook_ref.hook) do
              :ok ->
  ...
  defp hook_entity?(%Moto.Agent.Dsl.BeforeTurnHook{}), do: true
  defp hook_entity?(%Moto.Agent.Dsl.AfterTurnHook{}), do: true
  defp hook_entity?(%Moto.Agent.Dsl.InterruptHook{}), do: true
