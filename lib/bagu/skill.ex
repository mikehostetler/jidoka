defmodule Bagu.Skill do
  @moduledoc false

  alias Jido.AI.Skill

  @context_key :__bagu_skills__
  @state_key :__bagu_skill_runtime__
  ...
  def normalize_dsl(entries, base_dir) when is_list(entries) and is_binary(base_dir) do
    ...
    |> Enum.reduce_while({:ok, %{refs: [], load_paths: []}}, fn
      %Bagu.Agent.Dsl.SkillRef{skill: skill}, {:ok, acc} ->
      ...
      %Bagu.Agent.Dsl.SkillPath{path: path}, {:ok, acc} ->
  ...
  def on_before_cmd(agent, {:ai_react_start, %{tool_context: context} = params}, %{} = config)
    ...
    with {:ok, loaded_paths} <- ensure_loaded(config, loaded_paths),
         ...
         :ok <- Bagu.Debug.record_runtime_meta(context, %{skills: runtime_skill_info.names}) do
  ...
  defp ensure_unique_registry_name(name, acc) do
    if Map.has_key?(acc, name) do
      {:error, "skill names must be unique within a Bagu skill registry"}
    else
      :ok
    end
  end
