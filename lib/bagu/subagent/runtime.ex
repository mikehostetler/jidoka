defmodule Bagu.Subagent.Runtime do
  @moduledoc false

  alias Jido.AI.Request

  @request_meta_key :bagu_subagents
  ...
  def on_before_cmd(agent, {:ai_react_start, %{request_id: request_id} = params})
    ...
    context =
      ...
      |> Map.put(Bagu.Subagent.server_key(), self())
      |> Map.put_new(Bagu.Subagent.depth_key(), current_depth(context))
  ...
  @spec run_subagent_tool(Bagu.Subagent.t(), map(), map()) :: {:ok, map()} | {:error, term()}
  def run_subagent_tool(%Bagu.Subagent{} = subagent, params, context)
  ...
  @spec run_subagent(Bagu.Subagent.t(), map(), map()) :: {:ok, String.t()} | {:error, term()}
  def run_subagent(%Bagu.Subagent{} = subagent, params, context)
      when is_map(params) and is_map(context) do
    case execute_subagent(subagent, params, context) do
      {:ok, result, metadata} ->
        maybe_record_metadata(context, metadata)
  ...
  defp visible_result(%Bagu.Subagent{result: :structured}, result, metadata) do
    %{result: result, subagent: visible_metadata(metadata)}
  end

  defp visible_result(%Bagu.Subagent{}, result, _metadata), do: %{result: result}
  ...
  defp generated_child_id(%Bagu.Subagent{name: name}) do
    unique = System.unique_integer([:positive])
    "bagu-subagent-#{name}-#{unique}"
  end

  defp execute_subagent(%Bagu.Subagent{} = subagent, params, context) do
  ...
  defp delegate(
         %Bagu.Subagent{target: :ephemeral} = subagent,
         task,
         _parent_context,
         child_context,
         started_at
  ...
  defp delegate(
         %Bagu.Subagent{target: {:peer, peer_ref}} = subagent,
         task,
         parent_context,
         child_context,
         started_at
  ...
  defp ask_child(agent_module, pid, task, context, timeout) do
    if bagu_agent_module?(agent_module) do
      ask_bagu_child(agent_module, pid, task, context, timeout)
    else
      ask_compatible_child(agent_module, pid, task, context, timeout)
    end
  end

  defp ask_bagu_child(agent_module, pid, task, context, timeout) do
    child_opts = [context: context, timeout: timeout]

    with {:ok, prepared_opts} <-
           Bagu.Agent.prepare_chat_opts(child_opts, child_chat_config(agent_module)),
         request_opts <-
           Keyword.merge(
             prepared_opts,
             signal_type: "ai.react.query",
             source: "/bagu/subagent"
           ),
         {:ok, request} <- Request.create_and_send(pid, task, request_opts) do
      request
      |> await_child_request(agent_module, pid, timeout)
      |> normalize_bagu_child_result(pid, request.id, timeout)
    else
  ...
  defp normalize_bagu_child_result({:error, :timeout}, pid, request_id, timeout) do
    {:error, {:timeout, timeout}, request_id, child_request_meta(pid, request_id)}
  end

  defp normalize_bagu_child_result(await_result, pid, request_id, _timeout) do
    result =
      pid
      |> Bagu.finalize_chat_request(request_id, await_result)
  ...
  defp normalize_interrupt(interrupt) do
    {:ok, Bagu.Interrupt.new(interrupt)}
  rescue
  ...
  defp bagu_agent_module?(agent_module) do
  ...
  defp forwarded_context(context, policy) do
    context
    |> Bagu.Context.sanitize_for_subagent()
    |> apply_forward_context_policy(policy)
    |> Map.put(Bagu.Subagent.depth_key(), current_depth(context) + 1)
  end

  defp current_depth(context) when is_map(context) do
    case Map.get(context, Bagu.Subagent.depth_key(), 0) do
  ...
  defp maybe_record_metadata(context, metadata) when is_map(context) and is_map(metadata) do
    ...
    if is_pid(parent_server) and is_binary(request_id) do
      Bagu.Subagent.Metadata.insert(parent_server, request_id, metadata)
    end
  ...
  defp drain_request_meta(server, request_id) when is_pid(server) and is_binary(request_id) do
    Bagu.Subagent.Metadata.drain(server, request_id)
  end
  ...
  defp lookup_request_meta(server, request_id) when is_pid(server) and is_binary(request_id) do
    Bagu.Subagent.Metadata.lookup(server, request_id)
  end
  ...
  defp normalize_subagent_error(subagent, reason, context, metadata) do
    Bagu.Error.Normalize.subagent_error(reason,
      agent_id: subagent.name,
      target: subagent.target,
      request_id: Map.get(context, Bagu.Subagent.request_id_key()) || Map.get(metadata, :child_request_id),
      cause: reason
    )
  end
  ...
  defp context_keys(context) when is_map(context) do
    context
    |> Map.drop([
      Bagu.Subagent.request_id_key(),
      Bagu.Subagent.server_key(),
      Bagu.Subagent.depth_key()
    ])
    |> Map.keys()
