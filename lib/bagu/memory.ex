defmodule Bagu.Memory do
  ...
  @memory_context_key :__bagu_memory__
  ...
  def validate_dsl_entry(%Bagu.Agent.Dsl.MemoryMode{value: value}),
    do: validate_mode(value)

  def validate_dsl_entry(%Bagu.Agent.Dsl.MemoryNamespace{value: value}),
    do: validate_namespace_entry(value)

  def validate_dsl_entry(%Bagu.Agent.Dsl.MemorySharedNamespace{value: value}),
    do: validate_shared_namespace(value)

  def validate_dsl_entry(%Bagu.Agent.Dsl.MemoryCapture{value: value}),
    do: validate_capture(value)

  def validate_dsl_entry(%Bagu.Agent.Dsl.MemoryInject{value: value}),
    do: validate_inject(value)

  def validate_dsl_entry(%Bagu.Agent.Dsl.MemoryRetrieve{limit: limit}),
  ...
  def on_before_cmd(
    ...
    with {:ok, namespace} <- resolve_namespace(agent, context, config),
      ...
      {:error, reason} when is_binary(request_id) ->
        ...
        Logger.warning("Bagu memory retrieval failed: #{Bagu.format_error(error)}")

        failed_agent =
          agent
          |> Request.fail_request(request_id, error)
          |> put_request_memory_meta(request_id, %{error: error, warning: Bagu.format_error(error)})
      ...
      {:ok, result} ->
        with :ok <- remember_turn(agent, meta.namespace, user_record(meta, request_id)),
          ...
          {:error, reason} ->
            ...
            Logger.warning("Bagu memory capture failed: #{Bagu.format_error(error)}")
            ...
            {:ok,
             put_request_memory_meta(
               ...
               |> Map.put(:capture_warning, Bagu.format_error(error))
             ), directives}
        end
  ...
  defp put_entry(%Bagu.Agent.Dsl.MemoryMode{value: value}, acc), do: Map.put(acc, :mode, value)

  defp put_entry(%Bagu.Agent.Dsl.MemoryNamespace{value: value}, acc),
    do: Map.put(acc, :namespace, value)

  defp put_entry(%Bagu.Agent.Dsl.MemorySharedNamespace{value: value}, acc),
    do: Map.put(acc, :shared_namespace, value)

  defp put_entry(%Bagu.Agent.Dsl.MemoryCapture{value: value}, acc),
    do: Map.put(acc, :capture, value)

  defp put_entry(%Bagu.Agent.Dsl.MemoryInject{value: value}, acc),
    do: Map.put(acc, :inject, value)

  defp put_entry(%Bagu.Agent.Dsl.MemoryRetrieve{limit: limit}, acc),
  ...
  defp ensure_unique_entry(%module{} = entry, acc) do
    ...
    if Map.has_key?(acc, key) do
      {:error, "duplicate memory #{key} entry in Bagu agent"}
    else
      validate_dsl_entry(entry)
    end
  end

  defp dsl_entry_key(Bagu.Agent.Dsl.MemoryMode), do: :mode
  defp dsl_entry_key(Bagu.Agent.Dsl.MemoryNamespace), do: :namespace
  defp dsl_entry_key(Bagu.Agent.Dsl.MemorySharedNamespace), do: :shared_namespace
  defp dsl_entry_key(Bagu.Agent.Dsl.MemoryCapture), do: :capture
  defp dsl_entry_key(Bagu.Agent.Dsl.MemoryInject), do: :inject
  defp dsl_entry_key(Bagu.Agent.Dsl.MemoryRetrieve), do: :retrieve
  ...
  defp plugin_config(%{namespace: :per_agent}) do
    %{
      store: {Jido.Memory.Store.ETS, [table: :bagu_memory]},
      store_opts: [],
  ...
  defp resolve_namespace(agent, context, %{namespace: {:context, key}}) do
    case get_value(context, key) do
      nil ->
        {:error, Bagu.Error.missing_context(key, value: context)}
  ...
  defp memory_error(phase, reason, agent, request_id, config) do
    Bagu.Error.Normalize.memory_error(phase, reason,
  ...
  defp assistant_record(agent, meta, request_id, result) do
    ...
    %{
      ...
      tags: ["bagu", "conversation", "assistant"],
      source: "/bagu/agent",
      metadata:
  ...
  defp put_request_memory_meta(agent, request_id, memory_meta) when is_binary(request_id) do
    update_in(agent.state, [:requests, request_id], fn
      nil ->
        %{meta: %{bagu_memory: memory_meta}}

      request ->
        meta =
          request
          |> Map.get(:meta, %{})
          |> Map.put(:bagu_memory, memory_meta)
  ...
  defp get_request_memory_meta(agent, request_id) when is_binary(request_id) do
    get_in(agent.state, [:requests, request_id, :meta, :bagu_memory])
  end
