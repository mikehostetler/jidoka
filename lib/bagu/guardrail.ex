defmodule Bagu.Guardrail do
  @moduledoc """
  Thin wrapper for reusable Bagu guardrails.

  Bagu guardrails are published by name and expose a single `call/1` callback.
  ...
  @doc """
  Defines a reusable Bagu guardrail module.
  """
  ...
  @doc """
  Validates that a module implements the Bagu guardrail contract.
  """
  ...
  def validate_guardrail_module(module) when is_atom(module) do
    cond do
      ...
      missing = missing_functions(module) ->
        {:error, "guardrail #{inspect(module)} is not a valid Bagu guardrail; missing #{Enum.join(missing, ", ")}"}
  ...
  def normalize_available_guardrails(other),
    do:
      {:error,
       "available_guardrails must be a list of Bagu guardrail modules or a map of name => module, got: #{inspect(other)}"}
  ...
  defp ensure_unique_registry_name(name, acc) do
    if Map.has_key?(acc, name) do
      {:error, "guardrail names must be unique within a Bagu guardrail registry"}
    else
      :ok
    end
  end
