defmodule Bagu.Tool do
  @moduledoc """
  Thin wrapper around `Jido.Action` for defining Bagu-friendly tools.

  The goal is to keep the tool authoring surface small while still producing
  plain Jido actions underneath.

  Bagu tools are Zoi-first and Zoi-only for schema authoring. If a tool defines
  ...
  @doc """
  Defines a Bagu tool module backed by `Jido.Action`.
  """
  ...
  defmacro __using__(opts \\ []) do
    ...
    defaults = [
      name: module_name,
      description: "Bagu tool #{module_name}"
    ]

    quote location: :keep do
      use Jido.Action, unquote(Keyword.merge(defaults, opts))
      @after_compile Bagu.Tool
    end
  end
  ...
  @doc """
  Validates that a module behaves like a Bagu tool.
  """
    ...
    with :ok <- validate_action_module(module),
      ...
      {:error, message} ->
        if String.contains?(message, "valid action-backed tool") do
          {:error, String.replace(message, "valid action-backed tool", "valid Bagu tool")}
        else
          {:error, message}
        end
    end
    ...
    |> case do
      {:ok, names} ->
        if Enum.uniq(names) == names do
          ...
          {:error, "tool names must be unique within a Bagu agent"}
        end
  ...
  defp validate_tool_schema(module, function_name) do
    ...
    cond do
      ...
      true ->
        {:error,
         "tool #{inspect(module)} must use a Zoi schema for #{function_name}/0; NimbleOptions and raw JSON Schema maps are not supported in Bagu.Tool"}
    end
  rescue
    error ->
