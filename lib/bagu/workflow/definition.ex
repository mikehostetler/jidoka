defmodule Bagu.Workflow.Definition do
  @moduledoc false
  ...
  defp require_output!(nil, owner_module) do
    raise_error!(
      owner_module,
      "`output` is required for a Bagu workflow.",
  ...
  defp normalize_steps!([], owner_module) do
    raise_error!(
      owner_module,
      "A Bagu workflow must declare at least one step.",
      [:steps],
      [],
  ...
  defp normalize_step!(%Bagu.Workflow.Dsl.ToolStep{} = step, owner_module) do
  ...
  defp normalize_step!(%Bagu.Workflow.Dsl.FunctionStep{} = step, owner_module) do
  ...
  defp normalize_step!(%Bagu.Workflow.Dsl.AgentStep{} = step, owner_module) do
  ...
  defp validate_tool_target!(owner_module, step) do
    case Bagu.Tool.validate_action_module(step.module) do
      :ok ->
        :ok
      ...
      {:error, message} ->
        raise_error!(
          ...
          step.module,
          "Use a module defined with `use Bagu.Tool` or any valid Jido Action-backed module."
        )
    end
  end
    ...
    case Code.ensure_compiled(module) do
      {:module, ^module} ->
        if function_exported?(module, :start_link, 1) and function_exported?(module, :chat, 3) do
          ...
          raise_error!(
            owner_module,
            "Workflow agent step target is not a Bagu-compatible agent.",
            [:steps, step.name, :agent],
            module,
            "Use a compiled Bagu agent module or a compatible module exposing `start_link/1` and `chat/3`."
          )
        end
  ...
  defp validate_input_refs!(owner_module, input_schema, input_refs) do
    Enum.each(input_refs, fn key ->
      unless Bagu.Context.schema_has_key?(input_schema, key) do
        raise_error!(
          owner_module,
  ...
  defp collect_refs({:bagu_workflow_ref, :input, key}, acc),
    do: Map.update!(acc, :input, &[key | &1])

  defp collect_refs({:bagu_workflow_ref, :from, step, _path}, acc),
    do: Map.update!(acc, :from, &[step | &1])

  defp collect_refs({:bagu_workflow_ref, :context, key}, acc),
    do: Map.update!(acc, :context, &[key | &1])

  defp collect_refs({:bagu_workflow_ref, :value, _value}, acc), do: acc
  ...
  defp raise_error!(owner_module, message, path, value, hint) do
    raise Bagu.Workflow.Dsl.Error.exception(
            message: message,
            path: path,
            value: value,
            hint: hint,
