defmodule Bagu.Interrupt do
  @moduledoc """
  Structured human-in-the-loop or approval interrupt returned by Bagu agents.
  """
  ...
  @doc """
  Builds a normalized Bagu interrupt from a struct, map, or keyword list.
  """
  ...
  def new(attrs) when is_map(attrs) or is_list(attrs) do
    attrs =
      case Bagu.Context.coerce_map(attrs) do
        {:ok, normalized} ->
          normalized

        :error ->
          raise ArgumentError,
                "Bagu.Interrupt.new/1 expected a map or keyword list, got: #{inspect(attrs)}"
      end
  ...
  def new(other) do
    raise ArgumentError,
          "Bagu.Interrupt.new/1 expected a map, keyword list, or interrupt struct, got: #{inspect(other)}"
  end
