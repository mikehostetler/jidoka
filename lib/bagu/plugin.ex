defmodule Bagu.Plugin do
  @moduledoc """
  Thin wrapper around `Jido.Plugin` for defining Bagu-friendly plugins.
  ...
  Bagu plugins are the main extension point for adding capability around a Bagu
  ...
  @typedoc """
  A published Bagu plugin name.
  """
  @type name :: String.t()

  @typedoc """
  A registry of published Bagu plugin names to plugin modules.
  """
  @type registry :: %{required(name()) => module()}

  @doc """
  Defines a Bagu plugin module backed by `Jido.Plugin`.
  """
  ...
  defmacro __using__(opts \\ []) do
    ...
    defaults = [
      ...
      description: "Bagu plugin #{module_name}",
      actions: [],
  ...
  @doc """
  Validates that a module behaves like a Bagu plugin.
  """
  ...
  def validate_plugin_module(module) when is_atom(module) do
    cond do
      ...
      missing = missing_functions(module) ->
        {:error, "plugin #{inspect(module)} is not a valid Bagu plugin; missing #{Enum.join(missing, ", ")}"}
  ...
  @doc """
  Returns the published name for a validated Bagu plugin module.
  """
  ...
  @doc """
  Returns the published names for a list of Bagu plugin modules.
  """
    ...
    |> case do
      {:ok, names} ->
        if Enum.uniq(names) == names do
          ...
          {:error, "plugin names must be unique within a Bagu agent"}
        end
  ...
  def plugin_actions(module) when is_atom(module) do
    with :ok <- ensure_compiled_plugin(module),
         ...
         {:ok, _action_names} <- Bagu.Tool.action_names(actions) do
      {:ok, actions}
    else
      {:error, reason} ->
        {:error, reason}
  ...
  def normalize_available_plugins(other),
    do:
      {:error,
       "available_plugins must be a list of Bagu plugin modules or a map of name => module, got: #{inspect(other)}"}
