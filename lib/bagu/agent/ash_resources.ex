defmodule Bagu.Agent.AshResources do
  @moduledoc false
    ...
    cond do
      ...
      true ->
        with {:ok, _domain} <- domain(resource),
             ...
             {:ok, _names} <- Bagu.Tool.action_names(actions) do
          :ok
        end
    end
  rescue
  ...
  def expand(resources) when is_list(resources) do
    Enum.reduce_while(resources, {:ok, initial_acc()}, fn resource, {:ok, acc} ->
      with :ok <- validate_resource(resource),
           ...
           {:ok, resource_tool_names} <- Bagu.Tool.action_names(resource_actions),
  ...
  defp domain(resource) do
    case Ash.Resource.Info.domain(resource) do
      nil ->
        {:error, "Ash resource #{inspect(resource)} must declare a domain for Bagu ash_resource tools"}
  ...
  defp ensure_same_domain(expected, actual) do
    {:error,
     "Bagu ash_resource tools must all belong to the same Ash domain; got #{inspect(expected)} and #{inspect(actual)}"}
  end
