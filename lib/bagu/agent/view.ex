defmodule Bagu.Agent.View do
  @moduledoc """
  Thread-backed projections for UI-facing agent views.

  `Jido.Thread` is the canonical event log for an agent. Bagu projects that log
  ...
  def snapshot(server_or_id, opts) when is_list(opts) do
    case Jido.AgentServer.state(server_or_id) do
      ...
      {:error, reason} ->
        {:error, Bagu.Error.Normalize.debug_error(reason, target: server_or_id)}
    end
  end
