defmodule Bagu.Handoff.Tool do
  @moduledoc false
  ...
  @doc false
  @spec tool_module(module(), Bagu.Handoff.Capability.t(), non_neg_integer()) :: module()
  def tool_module(base_module, %Bagu.Handoff.Capability{} = handoff, index) do
    suffix =
      handoff.name
  ...
  @doc false
  @spec tool_module_ast(module(), Bagu.Handoff.Capability.t()) :: Macro.t()
  def tool_module_ast(tool_module, %Bagu.Handoff.Capability{} = handoff) do
    quote location: :keep do
      defmodule unquote(tool_module) do
        use Bagu.Tool,
          name: unquote(handoff.name),
          description: unquote(handoff.description),
          schema: unquote(Macro.escape(Bagu.Handoff.Capability.input_schema())),
          output_schema: unquote(Macro.escape(Bagu.Handoff.Capability.output_schema()))

        @handoff unquote(Macro.escape(handoff))

        @impl true
        def run(params, context) do
          Bagu.Handoff.Capability.run_handoff_tool(@handoff, params, context)
        end
      end
    end
  end
