defmodule Bagu.Handoff.Runtime do
  @moduledoc false

  @request_meta_key :bagu_handoffs

  @doc false
  @spec run_handoff_tool(Bagu.Handoff.Capability.t(), map(), map()) :: {:ok, map()} | {:error, term()}
  def run_handoff_tool(%Bagu.Handoff.Capability{} = capability, params, context)
    ...
    with {:ok, payload} <- normalize_payload(params),
      ...
      handoff =
        Bagu.Handoff.new(
          ...
          from_agent: Map.get(context, Bagu.Handoff.from_agent_key()),
          ...
          request_id: Map.get(context, Bagu.Handoff.request_id_key()),
          metadata: %{
      ...
      Bagu.Handoff.Registry.put_owner(conversation_id, handoff)
  ...
  def run_handoff_tool(%Bagu.Handoff.Capability{} = capability, _params, context) do
  ...
  def on_before_cmd(agent, {:ai_react_start, %{request_id: request_id} = params})
    ...
    context =
      context
      |> Map.put(Bagu.Handoff.request_id_key(), request_id)
      |> Map.put(Bagu.Handoff.server_key(), self())
      |> Map.put(Bagu.Handoff.from_agent_key(), from_agent(agent))
  ...
  defp conversation_id(context, %Bagu.Handoff.Capability{target: :auto}) do
    case context_value(context, Bagu.Handoff.context_key()) || context_value(context, :conversation) do
      value when is_binary(value) ->
        case String.trim(value) do
  ...
  defp resolve_target(%Bagu.Handoff.Capability{target: :auto, name: name}, conversation_id, _context) do
    {:ok, generated_agent_id(conversation_id, name)}
  end

  defp resolve_target(%Bagu.Handoff.Capability{target: {:peer, peer_id}}, _conversation_id, context) do
    resolve_peer_id(peer_id, context)
  end

  defp ensure_target_running(%Bagu.Handoff.Capability{target: :auto, agent: agent_module}, agent_id)
  ...
  defp ensure_target_running(%Bagu.Handoff.Capability{target: {:peer, _peer}}, agent_id)
       when is_binary(agent_id) do
    case Bagu.whereis(agent_id) do
  ...
  defp maybe_record_metadata(context, metadata) when is_map(context) and is_map(metadata) do
    parent_server = Map.get(context, Bagu.Handoff.server_key())
    request_id = Map.get(context, Bagu.Handoff.request_id_key())

    if is_pid(parent_server) and is_binary(request_id) do
      Bagu.Handoff.Metadata.insert(parent_server, request_id, metadata)
    end
  ...
  defp drain_request_meta(server, request_id) when is_pid(server) and is_binary(request_id) do
    Bagu.Handoff.Metadata.drain(server, request_id)
  end
  ...
  defp lookup_request_meta(server, request_id) when is_pid(server) and is_binary(request_id) do
    Bagu.Handoff.Metadata.lookup(server, request_id)
  end
  ...
  defp call_metadata(capability, %Bagu.Handoff{} = handoff, started_at, outcome) do
    %{
  ...
  defp error_metadata(capability, context, error, started_at) do
    %{
      ...
      target: capability.target,
      to_agent_id: nil,
      conversation_id: context_value(context, Bagu.Handoff.context_key()),
      request_id: Map.get(context, Bagu.Handoff.request_id_key()),
      duration_ms: System.monotonic_time(:millisecond) - started_at,
      outcome: {:error, error},
  ...
  defp normalize_handoff_error(capability, reason, context) do
    Bagu.Error.Normalize.handoff_error(reason,
      agent_id: Map.get(context, Bagu.Handoff.from_agent_key()),
      target: capability.target,
      field: :handoff,
      request_id: Map.get(context, Bagu.Handoff.request_id_key()),
      cause: reason
