defmodule Bagu.Inspection do
  ...
  @type running_agent_summary :: %{
          ...
          last_request: Bagu.Debug.summary() | nil
        }

  @spec inspect_agent(module() | struct() | pid() | String.t()) ::
          {:ok, agent_definition() | running_agent_summary()} | {:error, term()}
  def inspect_agent(%Bagu.ImportedAgent{} = agent),
    do: {:ok, Bagu.ImportedAgent.definition(agent)}

  def inspect_agent(module) when is_atom(module) do
    _ = Code.ensure_loaded(module)

    cond do
      function_exported?(module, :__bagu__, 0) ->
        {:ok, module.__bagu__()}

      function_exported?(module, :__bagu_definition__, 0) ->
        {:ok, module.__bagu_definition__()}

      true ->
        {:error,
         Bagu.Error.config_error("Module is not a Bagu agent.",
           field: :agent,
           value: module,
           details: %{operation: :inspect_agent, reason: :not_bagu_agent, cause: :not_bagu_agent}
         )}
    end
  end
  ...
  def inspect_workflow(module) when is_atom(module), do: Bagu.Workflow.inspect_workflow(module)

  @spec inspect_request(pid() | String.t() | Jido.Agent.t()) ::
          {:ok, Bagu.Debug.summary()} | {:error, term()}
  def inspect_request(server_or_agent), do: Bagu.Debug.request_summary(server_or_agent)

  @spec inspect_request(pid() | String.t() | Jido.Agent.t(), String.t()) ::
          {:ok, Bagu.Debug.summary()} | {:error, term()}
  def inspect_request(server_or_agent, request_id),
    do: Bagu.Debug.request_summary(server_or_agent, request_id)
  ...
  defp request_summary(agent, request_id) when is_binary(request_id) do
    case Request.get_request(agent, request_id) do
      ...
      _request ->
        case Bagu.Debug.request_summary(agent, request_id) do
          {:ok, summary} -> summary
          _ -> nil
        end
    end
  ...
  defp runtime_definition(runtime_module) when is_atom(runtime_module) do
    if function_exported?(runtime_module, :__bagu_definition__, 0) do
      runtime_module.__bagu_definition__()
    else
      nil
    end
  end
  ...
  defp runtime_owner_module(runtime_module) when is_atom(runtime_module) do
    if function_exported?(runtime_module, :__bagu_owner_module__, 0) do
      runtime_module.__bagu_owner_module__()
    else
      nil
    end
  end
