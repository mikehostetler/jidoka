defmodule Bagu.MCP do
  @moduledoc """
  Bagu-facing MCP endpoint and tool-sync helpers.

  Bagu treats MCP servers as a first-class tool source for agents. MCP endpoints
  ...
  Bagu uses this for inline `mcp_tools` endpoint declarations.
  """
  ...
  @doc """
  Syncs an MCP endpoint's tools into a running Bagu/Jido.AI agent.
  ...
  def validate_dsl_entry(%Bagu.Agent.Dsl.MCPTools{} = entry) do
  ...
  def on_before_cmd(agent, {:ai_react_start, _params} = action, config) when is_list(config) do
    ...
    :ok =
      Bagu.Debug.record_runtime_meta(
        context,
        %{
  ...
  defp normalize_dsl_entry(%Bagu.Agent.Dsl.MCPTools{} = entry) do
    normalize_entry(%{
  ...
  defp endpoint_attrs(entry) do
    %{
      transport: Map.get(entry, :transport, Map.get(entry, "transport")),
      client_info: Map.get(entry, :client_info, Map.get(entry, "client_info", %{name: "bagu"})),
  ...
  defp sync_module do
    Application.get_env(:bagu, :mcp_sync_module, Bagu.MCP.Sync)
  end
  ...
  defp format_error(entry, reason) do
    error =
      Bagu.Error.Normalize.mcp_error(reason,
    ...
    %{
      ...
      prefix: entry.prefix,
      reason: error,
      message: Bagu.format_error(error)
    }
  end
  ...
  defp normalize_mcp_result({:error, reason}, context) do
    {:error, Bagu.Error.Normalize.mcp_error(reason, context)}
  end

  defp normalize_mcp_result(other, context) do
    {:error, Bagu.Error.Normalize.mcp_error({:invalid_result, other}, context)}
  end
