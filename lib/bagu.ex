defmodule Bagu do
  @moduledoc """
  Minimal runtime facade for starting and discovering Bagu agents.
  """
  ...
  @doc """
  Returns Bagu-owned model aliases from application config.

  These aliases are defined under `config :bagu, :model_aliases`.
  """
  @spec model_aliases() :: %{optional(atom()) => term()}
  def model_aliases do
    case Application.get_env(:bagu, :model_aliases, %{}) do
  ...
  @doc """
  Normalizes a model input using Bagu aliases first, then Jido.AI.
  """
  ...
  @doc """
  Starts an agent under the shared `Bagu.Runtime` instance.
  """
  ...
  def start_agent(agent, opts), do: Bagu.Runtime.start_agent(agent, opts)

  @doc """
  Stops an agent by PID or registered ID.
  """
  @spec stop_agent(pid() | String.t(), keyword()) :: :ok | {:error, :not_found}
  def stop_agent(pid_or_id, opts \\ []), do: Bagu.Runtime.stop_agent(pid_or_id, opts)

  @doc """
  Looks up a running agent by ID.
  """
  @spec whereis(String.t(), keyword()) :: pid() | nil
  def whereis(id, opts \\ []), do: Bagu.Runtime.whereis(id, opts)

  @doc """
  Lists all running agents.
  """
  @spec list_agents(keyword()) :: [{String.t(), pid()}]
  def list_agents(opts \\ []), do: Bagu.Runtime.list_agents(opts)

  @doc """
  Imports a constrained Bagu agent from a map, JSON string, or YAML string.
  ...
  @doc """
  Imports a constrained Bagu agent and raises on failure.
  """
  ...
  @doc """
  Imports a constrained Bagu agent from a `.json`, `.yaml`, or `.yml` file.
  """
  ...
  @doc """
  Imports a constrained Bagu agent from a file and raises on failure.
  """
  ...
  @doc """
  Encodes an imported Bagu agent as JSON or YAML.
  """
  ...
  @doc """
  Formats Bagu error terms for humans.

  Use this helper when presenting Bagu errors in CLIs, demos, logs, or tests.
  """
  @spec format_error(term()) :: String.t()
  def format_error(reason), do: Bagu.Error.format(reason)

  @doc """
  Encodes an imported Bagu agent as JSON or YAML and raises on failure.
  """
  ...
  @doc """
  Sends a chat request to a running Bagu agent and waits for the result.
  ...
  defp chat_config(server) do
    case Jido.AgentServer.state(server) do
      {:ok, %{agent_module: runtime_module}} when is_atom(runtime_module) ->
        if function_exported?(runtime_module, :__bagu_definition__, 0) do
          definition = runtime_module.__bagu_definition__()
  ...
  @doc """
  Returns Bagu's inspection view of an agent definition or running agent.

  Accepted inputs:

  - a compiled Bagu agent module
  - an imported Bagu agent struct
  ...
  def inspect_agent(target), do: Bagu.Inspection.inspect_agent(target)

  @doc """
  Returns Bagu's inspection view of a compiled workflow definition.
  """
  @spec inspect_workflow(module()) :: {:ok, map()} | {:error, term()}
  def inspect_workflow(workflow_module), do: Bagu.Inspection.inspect_workflow(workflow_module)

  @doc """
  Returns a summary for the latest request on a running Bagu agent.
  """
  @spec inspect_request(pid() | String.t() | Jido.Agent.t()) ::
          {:ok, map()} | {:error, term()}
  def inspect_request(target), do: Bagu.Inspection.inspect_request(target)
  ...
  @spec inspect_request(pid() | String.t() | Jido.Agent.t(), String.t()) ::
          {:ok, map()} | {:error, term()}
  def inspect_request(target, request_id), do: Bagu.Inspection.inspect_request(target, request_id)
  ...
  def chat_request(server, message, opts) when is_binary(message) and is_list(opts) do
    ...
    request_opts = Keyword.merge(opts, signal_type: "ai.react.query", source: "/bagu/agent")
    ...
    case Jido.AgentServer.state(server) do
      {:ok, %{agent: agent}} ->
        case Request.get_request(agent, request_id) do
          %{meta: %{bagu_guardrails: %{interrupt: interrupt}}} ->
            {:error, {:interrupt, interrupt}}

          %{meta: %{bagu_guardrails: %{error: error}}} ->
            {:error, error}
